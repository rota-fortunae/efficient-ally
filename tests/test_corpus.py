...
KNOWN_MISSES = {
    ...
    "bubble_sort.fast": "operation costs: sorted() is n log n",
    "common_elements.fast": "operation costs: set(b) is O(len(b))",
    ...
    "first_repeat.slow": "operation costs: `in items[:i]` slices and scans",
    ...
    "group_anagrams.group": "operation costs: sorted(word) and str.join",
    "has_duplicates.fast": "operation costs: set(items) is O(len(items))",
    "k_smallest.slow": "collection sizes + loop bounds: len(remaining) = len(nums); min(k, x) <= k",
    "k_smallest.fast": "operation costs: heapq.nsmallest is n log k",
    "known_words.fast": "operation costs: set(vocabulary) is O(len(vocabulary))",
    "matrix_multiply.multiply": "aliases: n = len(a)",
    "max_subarray.fast": "collection sizes: len(nums[1:]) is about len(nums)",
    "min_gap.fast": "operation costs + collection sizes: sorted(); len(ordered) = len(nums)",
    "normalize.slow": "operation costs: max(values) inside the comprehension",
    "pascal_triangle.triangle": "loop bounds: j < i < n",
    "power.fast": "while loops: exponent halves each time -> log",
    "prefix_sums.slow": "operation costs: sum() of a slice inside the comprehension",
    ...
    "print_queue.fast": "while loops: runs until `queue` (from jobs) is empty",
    "repeated_min.slow": "while loops + collection sizes: runs until `remaining` is empty",
    "repeated_min.fast": "operation costs + collection sizes: heappop(); len(heap) = len(nums)",
    "reverse_string.slow": "string building: prepending (`result = char + result`) isn't seen",
    "reverse_string.fast": "operation costs: slicing copies the string",
    "rotate.fast": "operation costs: slicing and + copy the list",
    "shared_tags.fast": "operation costs: set() and & are linear",
    "sorted_queries.slow": "operation costs: sorted() inside the loop",
    "sorted_queries.fast": "operation costs: sorted() is n log n",
